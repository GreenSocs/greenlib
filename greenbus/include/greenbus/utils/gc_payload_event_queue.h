...
/*****************************************************************************
  ...
  Christian Schröder, 2007-06
  GreenControl payload event queue (gc_payload_event_queue.h)
    Only works in call_mode and is usable earlyer 
    (not recently as end_of_elaboration)
 *****************************************************************************/

// DONOTRELICENSE

#ifndef CALL_PAYLOAD_EVENT_QUEUE_H
#define CALL_PAYLOAD_EVENT_QUEUE_H
...
public:
  struct element
  {
    ...
    element(PAYLOAD p, sc_time t, unsigned long long d): p(p),t(t),d(d) {}
  };
...
//RG: wouldn't it be faster if the notifys took references?
  ...
  template<typename PAYLOAD>
  class call_payload_event_queue_if : public virtual sc_interface // CS
  {
  public:
  ...
  template<typename PAYLOAD>
  class call_payload_event_queue_output_if : public virtual sc_interface
  {
  public:
    virtual void notify (PAYLOAD& p) =0;
  };
  ...
  template<typename PAYLOAD>
  class call_peq_outport_binder : public sc_module, public call_payload_event_queue_output_if<PAYLOAD>
  {
  public:
    SC_CTOR(call_peq_outport_binder){}
...
//---------------------------------------------------------------------------
template<typename PAYLOAD>
class call_payload_event_queue: // CS
  public call_payload_event_queue_if<PAYLOAD>, // CS
  public sc_module
{
public:

  sc_port<call_payload_event_queue_output_if<PAYLOAD>,0> out_port;
  
  SC_HAS_PROCESS( call_payload_event_queue ); // CS
  
  call_payload_event_queue(); // CS
  call_payload_event_queue( sc_module_name name_ ); // CS
  virtual ~call_payload_event_queue(); // CS
...
private:
  ...
  call_peq_outport_binder<PAYLOAD> dummy_bind;
  //bool callMode; // CS
  
private:
  
  call_timeorderedlist<PAYLOAD> m_ppq;
...
template<typename PAYLOAD>
void call_payload_event_queue<PAYLOAD>::notify (PAYLOAD p, const sc_time& when) // CS
{
...
template<typename PAYLOAD>
void call_payload_event_queue<PAYLOAD>::notify (PAYLOAD p) // CS
{
  //if (callMode){ // CS
    out_port[1]->notify(p);
  //}
...
template<typename PAYLOAD>
void call_payload_event_queue<PAYLOAD>::notify (PAYLOAD p, double when, sc_time_unit base ) // CS
{
  notify( p, sc_time(when,base) );
}

// CS: komplett entfernen
...
/*template<typename PAYLOAD>
  const sc_event& call_payload_event_queue<PAYLOAD>::default_event() const // CS
  {
  return m_e; 
  }*/
...
/// advance PEQ to now
template<typename PAYLOAD>
void call_payload_event_queue<PAYLOAD>::advance() // CS
{
...
// but this should be less then an assert.
/*template<typename PAYLOAD>
  PAYLOAD &call_payload_event_queue<PAYLOAD>::get_payload() // CS
  {
  advance();
  return m_ppq.top();
  }*/
...
// but this should be less then an assert.
/*template<typename PAYLOAD>
  void call_payload_event_queue<PAYLOAD>::wait(PAYLOAD p) // CS
  {
  // CS TODO: komplett entfernen
...
# define DUST_PEQ_GS_ATOM_NAME "GS_ATOM"
...
template<typename PAYLOAD>
call_payload_event_queue<PAYLOAD>::call_payload_event_queue() // CS
  : sc_module( sc_gen_unique_name( "call_payload_event_queue" ) ),
    dummy_bind("dummy_bind"), m_delta(0),m_time(0-10,SC_NS)
#ifdef DUST_RECORD_TRANSACTIONS
...
template<typename PAYLOAD>
call_payload_event_queue<PAYLOAD>::call_payload_event_queue( sc_module_name name_ ) // CS
  : sc_module( name_ ),
    dummy_bind("dummy_bind"), m_delta(0),m_time(0-10,SC_NS)
#ifdef DUST_RECORD_TRANSACTIONS
   ,peq_stream(DUST_PEQ_STREAM_NAME, DUST_PEQ_STREAM_KIND),
...
/*template<typename PAYLOAD>
  void call_payload_event_queue<PAYLOAD>::end_of_elaboration(){ // CS
...
template<typename PAYLOAD>
void call_payload_event_queue<PAYLOAD>::cancel_all() // CS
{
...
template<typename PAYLOAD>
void call_payload_event_queue<PAYLOAD>::fire_event() // CS
{
#ifdef DUST_RECORD_TRANSACTIONS
  ...
  h.record_attribute(DUST_PEQ_GS_ATOM_NAME, *(p.first));
  ...
  advance();
  sc_time now=sc_time_stamp();  
  if ( m_ppq.getSize() > 1 ) { // TODO: wenn vorheriges PAYLOAD im selben Timepoint (nur älterer Delta) ist, ist es hier noch da!!!
...
template<typename PAYLOAD>
void call_payload_event_queue<PAYLOAD>::fec(){ // CS
  assert(m_ppq.getSize()); //there has to be something inside
  sc_time now=sc_time_stamp();
  sc_time top=m_ppq.top_time();
  unsigned long long delta=sc_delta_count();
...
#endif // CALL_PAYLOAD_EVENT_QUEUE_H
