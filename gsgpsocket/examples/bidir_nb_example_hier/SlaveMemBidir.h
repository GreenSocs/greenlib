...
public:
   ...
   */
  SlaveMemBidir(sc_core::sc_module_name name_, int delay_=3) :
  ...
  case GenericPhase::DataValid:
    {
      ...
      GS_DUMP("Got " << (unsigned)tah->getMBurstLength() <<" data bytes sent to addr=0x" <<(std::hex)
              <<(gs_uint64)tah->getMAddr()<<" (local addr=0x"<<(gs_uint64)(tah->getMAddr()-target_port.base_addr) <<(std::dec)<<"):");
      for (unsigned int i=0; i<tah->getMBurstLength(); i++){
        MEM[i+(tah->getMAddr()-target_port.base_addr)]=my_data[i];
      }
...
int SlaveMemBidir::IPmodel( accessHandle t)
{
  if (t->getMCmd()==Generic_MCMD_RD){
    GS_DUMP("processing (blocking or nb) READ: burstlength="<<(unsigned)t->getMBurstLength()
            <<", local addr=0x"<<(std::hex)<< (gs_uint64)(t->getMAddr()-target_port.base_addr) <<(std::dec)<<", data=");
    std::cout << "         stored data to transmit: ";
    for (unsigned int i=0; i<t->getMBurstLength(); i++) {
      std::cout << (unsigned int)MEM[i+(t->getMAddr()-target_port.base_addr)] << " ";
    } std::cout << std::endl;
    for (unsigned int i=0; i<t->getMBurstLength(); i++) {
      t->getSData()[i] = MEM[i+(t->getMAddr()-target_port.base_addr)];
    }
    return m_delay;
  }
  ...
  if (t->getMCmd()==Generic_MCMD_WR){
    ...
    for (unsigned int i=0; i<t->getMBurstLength(); i++) {
      ...
      MEM[i+(t->getMAddr()-target_port.base_addr)]=my_data[i];
    } std::cout << std::endl;
    return m_delay*2;
  }
...
public:
  ...
  SlaveMemBidirHier(sc_core::sc_module_name name_, int delay_=3) :
    sc_module(name_),
    target_port("tport"),
    child(new SlaveMemBidir((std::string(name_)+"_child").c_str(), delay_))
  {
    // Bind hierarchically
    target_port.bind_to_inner(child->target_port);
  }
